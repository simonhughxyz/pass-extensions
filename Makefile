PROJECT_NAME := "pass-extensions"
PREFIX := ~/.password-store/.extensions
SRC_PREFIX := ./src
SRC := $(patsubst $(SRC_PREFIX)/%, %, $(wildcard $(SRC_PREFIX)/*))
INSTALL_PATH := $(patsubst %, $(PREFIX)/%, $(SRC))
...
.PHONY: uninstall
uninstall: $(patsubst $(PREFIX)/%, uninstall-%, $(INSTALL_PATH))
	@echo
